...
export const VerdictProgress = ({
  ...
  const percentages = useMemo(() => {
    ...
    return {
      ...
      "Insufficient Information": (distribution["Insufficient Information"] / verdicts.length) * 100,
      "Conflicting Evidence": (distribution["Conflicting Evidence"] / verdicts.length) * 100,
    };
    ...
    <TooltipProvider>
      ...
      >
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-medium text-neutral-900 my-2.5 mt-6">Verification Results</h3>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="cursor-help">
                  <InfoIcon className="h-3.5 w-3.5 text-neutral-400 hover:text-neutral-500 transition-colors" />
                </div>
              </TooltipTrigger>
              <TooltipContent side="top" className="text-xs bg-black text-white border-0 px-3 py-1.5">
        ...
        >
          {Object.entries(verdictConfig).map(([verdict, config]) => {
            ...
            const formattedPercent = percent > 0 ? `${Math.round(percent)}%` : "0%";
                ...
                <TooltipTrigger asChild>
                  <div className="flex items-center gap-1.5 cursor-default">
                    ...
                    <div className={cn(
                      ...
                      <span className="ml-1 tabular-nums">{formattedPercent}</span>
                    </div>
                  </div>
                </TooltipTrigger>
                <TooltipContent side="bottom" className="text-xs bg-black text-white border-0 px-2.5 py-1.5">
